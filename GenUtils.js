    ...
    statics: {
        ...
        */
        SplitDataUsingInputFormatters: function (p_strData, p_arrParams) {
            ...
            if (this.pvtIsCopiedDataFromSpreadSheet(LFinalResult)) {
                ...
                console.log(LControllerReference)
                return null
            }
            ...
            //     LFinalResult = LMe.pvtRemoveSelectedTag(LFinalResult, /\r?\n/);
            ...
            // else {
                ...
                if (Lset.has(LConstraintsAvailable.NEWLINE_PARAM)) {
                    // LFinalResult = LMe.pvtRemoveSelectedTag(LFinalResult,/<p[^>]*>(?:\s|\S)*?<\/p>/gi);
        ...
        pvtIsCopiedDataFromSpreadSheet: function (p_strData) {
            ...
            // let TotalArr = p_strData[0].match(/<table[^>]*>[\s\S]*?<\/table>/gi)

            return p_strData[0].trim().startsWith('<table') && p_strData[0].trim().endsWith('</table>');
        ...
        */
        ...
        pvtRemoveSelectedTag: function (p_arrData, p_regTag) { 
            return p_arrData.flatMap(str => str.split(p_regTag).map(str => str.trim()).filter(Boolean));
